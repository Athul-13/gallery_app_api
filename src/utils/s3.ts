...
/**
 * S3 utility functions for image upload, delete, and update operations
 */
...
export const validateImageFile = (
  ...
  // Get max size from parameter or env (convert string to number)
  const maxSizeBytes = maxSize || parseInt(AWS_MAX_FILE_SIZE, 10)
  ...
  const allowedMimeTypes = allowedTypes || AWS_ALLOWED_MIME_TYPES.split(',').map((t) => t.trim())
 ...
 */
const generateS3Key = (userId: string, filename: string, timestamp?: number): string => {
 ...
 */
const constructS3Url = (key: string, bucket: string, region: string): string => {
  // If custom bucket URL is provided, use it
  if (AWS_S3_BUCKET_URL) {
    return `${AWS_S3_BUCKET_URL.replace(/\/$/, '')}/${key}`
  }
...
export const uploadImageToS3 = async (
  ...
  try {
    // Generate S3 key
    const filename = title ? `${title}${file.originalname.substring(file.originalname.lastIndexOf('.'))}` : file.originalname
    ...
    const url = constructS3Url(key, AWS_S3_BUCKET_NAME, AWS_REGION)
...
export const uploadMultipleImagesToS3 = async (
  ...
  files.forEach((file, index) => {
    const validation = validateImageFile(file)
    if (!validation.isValid) {
      throw createError(400, `Invalid file at index ${index}: ${file.originalname} - ${validation.error}`)
    }
  })
 ...
 */
export const deleteImageFromS3 = async (s3KeyOrUrl: string): Promise<S3DeleteResult> => {
  try {
    // Extract key from URL if URL provided
    let key = s3KeyOrUrl
    if (s3KeyOrUrl.startsWith('http://') || s3KeyOrUrl.startsWith('https://')) {
      const extractedKey = extractS3KeyFromUrl(s3KeyOrUrl)
      if (!extractedKey) {
...
export const updateImageInS3 = async (
  ...
  try {
    ...
    if (oldS3KeyOrUrl.startsWith('http://') || oldS3KeyOrUrl.startsWith('https://')) {
      const extractedKey = extractS3KeyFromUrl(oldS3KeyOrUrl)
      if (!extractedKey) {
    ...
    const uploadResult = await uploadImageToS3(file, userId, title)
